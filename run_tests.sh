...
if [[ -z "$DATA_DIR" ]]; then
    ...
    check_gold_table_duckdb() {
        ...
        if $DUCKDB_AVAILABLE; then
            ...
            local row_count
            row_count=$(duckdb -csv -noheader -c "INSTALL delta; LOAD delta; SELECT COUNT(*) FROM delta_scan('${table_path}');" 2>/dev/null | grep -E '^[0-9]+$' | head -1 || echo "ERROR")
...
DUCKDB_EOF
        ...
        QUERY_OUTPUT=$(echo "$DUCKDB_SCRIPT" | duckdb -csv -noheader 2>/tmp/duckdb_queries.log || true)
        ...
        if [[ $? -ne 0 ]] || grep -q "Error" /tmp/duckdb_queries.log 2>/dev/null; then
            ...
            Q1_ROWS=$(echo "$QUERY_OUTPUT" | grep "^Q1" | awk -F',' '{print $NF}' || echo "0")
            if [[ "$Q1_ROWS" == "4" ]]; then
            ...
            Q2_ORPHANS=$(echo "$QUERY_OUTPUT" | grep "^Q2" | awk -F',' '{print $NF}' || echo "-1")
            ...
            Q3_ROWS=$(echo "$QUERY_OUTPUT" | grep "^Q3" | awk -F',' '{print $NF}' || echo "0")
...
src = report.get("source_record_counts", {})
gold = report.get("gold_layer_record_counts", {})
issues = report.get("dq_issues", [])
if not isinstance(issues, list):
    print("INVALID_DQ_ISSUES: dq_issues must be a JSON array")
    sys.exit(1)

txn_raw = src.get("transactions_raw", 0)
txn_gold = gold.get("fact_transactions", 0)
flagged = sum(i.get("records_affected", 0) for i in issues)
print(f"OK: src_txn={txn_raw}, gold_txn={txn_gold}, issues={len(issues)}, flagged={flagged}")
PYEOF
)
    ...
    if [[ "$STAGE" -eq 3 ]]; then
        ...
        for table in "${STREAM_TABLES[@]}"; do
            ...
            if [[ "$PARQUET_COUNT" -gt 0 ]]; then
                if $DUCKDB_AVAILABLE; then
                    ROW_COUNT=$(duckdb -csv -noheader -c "INSTALL delta; LOAD delta; SELECT COUNT(*) FROM delta_scan('${TABLE_PATH}');" 2>/dev/null | grep -E '^[0-9]+$' | head -1 || echo "0")
